"""
Export a KaroSpace viewer for the breast-cancer (rep1) Xenium dataset that carries
BOTH a DAPI morphology image and a post-Xenium H&E image.
...
between DAPI and H&E. They share one alignment — align once (or use ✨ Auto-align),
then fine-tune and "Save session" to persist it.

Usage:
    python examples/xenium_dapi_he_breast_cancer_rep1.py
    XENIUM_DAPI_HE_BREAST_H5AD=/path/to/file.h5ad python examples/xenium_dapi_he_breast_cancer_rep1.py
"""
...
H5AD_PATH = os.environ.get(
    "XENIUM_DAPI_HE_BREAST_H5AD",
    "/Users/chrislangseth/work/karolinska_institutet/projects/KaroSpaceDataWrangling/"
    "data/processed/xenium-dapi-he/breast_cancer_rep1.companion.ready.h5ad",
)

OUTPUT_DIR = REPO_ROOT
IMAGE_DIR = OUTPUT_DIR / "xenium_dapi_he_breast_images"
SIDECAR_OUTPUT = str(OUTPUT_DIR / "xenium_dapi_he_breast_cancer_rep1.html")
PACKAGE_OUTPUT = str(OUTPUT_DIR / "xenium_dapi_he_breast_cancer_rep1.karospace")
GENE_AUX_PATH = str(OUTPUT_DIR / "xenium_dapi_he_breast_cancer_rep1.genes.json")
...
def main() -> None:
    if not Path(H5AD_PATH).exists():
        raise SystemExit(
            f"H5AD not found: {H5AD_PATH}\nSet XENIUM_DAPI_HE_BREAST_H5AD or edit the path above."
        )
    ...
    common_kwargs = dict(
        ...
        title="Breast Cancer Xenium (rep1) — DAPI + H&E",
        min_panel_size=140,
        spot_size="auto",
        outline_by=None,
