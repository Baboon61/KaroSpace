"""
Export a KaroSpace viewer for the TWO-sample breast-cancer Xenium dataset
(breast_cancer_rep1 + breast_cancer_rep2). Each sample carries BOTH a DAPI
morphology image and a post-Xenium H&E image, resampled onto the same DAPI hires
grid (pixel aligned).

Because there are two sections and EACH has its own image pair, the overlays are
attached PER SECTION (rep1's images to rep1, rep2's to rep2). In the viewer, open
a section and use the dropdown in the "H&E Overlay" controls to switch DAPI <-> H&E
(they share one alignment), or use ✨ Auto-align.

Usage:
    python examples/xenium_dapi_he_breast_cancer_2samples.py
    XENIUM_DAPI_HE_BREAST2_H5AD=/path/to/file.h5ad python examples/xenium_dapi_he_breast_cancer_2samples.py
"""
...
H5AD_PATH = os.environ.get(
    "XENIUM_DAPI_HE_BREAST2_H5AD",
    "/Users/chrislangseth/work/karolinska_institutet/projects/KaroSpaceDataWrangling/"
    "data/processed/xenium-dapi-he/breast_cancer_2samples.companion.ready.h5ad",
)

OUTPUT_DIR = REPO_ROOT
IMAGE_DIR = OUTPUT_DIR / "xenium_dapi_he_breast2_images"
SIDECAR_OUTPUT = str(OUTPUT_DIR / "xenium_dapi_he_breast_cancer_2samples.html")
PACKAGE_OUTPUT = str(OUTPUT_DIR / "xenium_dapi_he_breast_cancer_2samples.karospace")
GENE_AUX_PATH = str(OUTPUT_DIR / "xenium_dapi_he_breast_cancer_2samples.genes.json")
...
def main() -> None:
    if not Path(H5AD_PATH).exists():
        raise SystemExit(
            f"H5AD not found: {H5AD_PATH}\nSet XENIUM_DAPI_HE_BREAST2_H5AD or edit the path above."
        )
    ...
    print(
        ...
        "switch DAPI <-> H&E. Align once (or ✨ Auto-align), then Save session to persist it."
    )
