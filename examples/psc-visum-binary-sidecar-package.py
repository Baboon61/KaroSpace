...
This script is configured for the PSC Visium companion-ready h5ad and writes:
...
SIDECAR_OUTPUT = "psc-visum-binary-sidecar.html"
PACKAGE_OUTPUT = "psc-visum-binary.karospace"
GENE_AUX_PATH = "psc-visum-binary.genes.json"

if not Path(H5AD_PATH).exists():
    raise SystemExit(
        "PSC Visium h5ad not found. Set PSC_VISUM_H5AD_PATH before running "
        "examples/psc-visum-binary-sidecar-package.py."
    )

adata = _read_h5ad_with_fallback(H5AD_PATH)
if "feature_name" not in adata.var.columns:
    raise SystemExit("PSC Visium h5ad is missing adata.var['feature_name'].")
